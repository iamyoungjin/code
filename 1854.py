...
입력
첫째 줄에 n, m, k가 주어진다. 
(1 ≤ n ≤ 1000, 0 ≤ m ≤ 2000000, 1 ≤ k ≤ 100) n과 m은 각각 김 조교가 여행을 고려하고 있는 도시들의 개수와, 도시 간에 존재하는 도로의 수이다.
...
i번 도시에서 i번 도시로 가는 최단경로는 0이지만, 일반적인 k번째 최단경로는 0이 아닐 수 있음에 유의한다. 또, k번째 최단경로가 존재하지 않으면 -1을 출력한다.
...
def dijkstra(start):
	...
	while q:
		...
		for x,y in graph[now]:
			new_dist = y+dist
			if new_dist < distance[x][k-1]:
				distance[x][k-1] = new_dist
				distance[x].sort()
				heapq.heappush(q,[new_dist,x])
...
if __name__ == '__main__':
	...
	for _ in range(m):
		a,b,c = map(int,input().split()) # a 에서 b로 갈때 c시간 
		graph[a].append((b,c))
